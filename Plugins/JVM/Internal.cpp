...
Internal::Internal(JVM* parent) : m_parent(parent)
{
    ...
    LOG_INFO("Classpath: %s", classpath);
    ...
    LOG_INFO("JNI checking turned on (Debug Build).");
#else
    ...
    LOG_INFO("Creating the Virtual Machine.");
    ...
    DoAttached([&](JavaVM*, JNIEnv* env) {
        LOG_INFO("Looking up required class and method IDs.");
