...
#define REGISTER(func) \
    ...
    REGISTER(GetEncounterGeometry);
    REGISTER(SetEncounterGeometry);
...
ArgumentStack Encounter::GetEncounterGeometry(ArgumentStack&& args)
{
    std::string retVal;

    if (auto *pObject = encounter(args))
    {
        if (auto *pEncounter = Utils::AsNWSEncounter(pObject))
        {
        ...
        {
            LOG_WARNING("NWNX_Encounter_GetEncounterGeometry() called on non encounter object.");
        }
    }
...
ArgumentStack Encounter::SetEncounterGeometry(ArgumentStack&& args)
{
    if (auto *pObject = encounter(args))
    {
        ...
        if (auto *pEncounter = Utils::AsNWSEncounter(pObject))
        {
            ...
            do {
                ...
                if (cnt != 2 && cnt != 3)
                {
                    LOG_NOTICE("NWNX_Encounter_SetEncounterGeometry() invalid geometry string at: %s", str);
                    break;
                }
            ...
            if (vecVerts.size() > 2)
            {
                ...
                if (pEncounter->m_pvActivateVertices)
                    delete[] pEncounter->m_pvActivateVertices;
                if (pEncounter->m_pnOutlineVertices)
            ...
            {
                LOG_WARNING("NWNX_Encounter_SetEncounterGeometry() called with less than 3 vertices.");
            }
        }
        else
        {
            LOG_WARNING("NWNX_Encounter_SetEncounterGeometry() called on non encounter object.");
        }
    }
